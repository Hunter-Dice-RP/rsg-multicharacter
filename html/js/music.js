...
function setupCharacters(characters) { 
    ...
    for (let i = 1; i <= NChar; i++) {
        ...
        let html;
        if (char && char.charinfo) { html = `<div class="character" id="char-${i}" data-cid="${i}"><span id="slot-name">${char.charinfo.firstname} ${char.charinfo.lastname}<span id="cid">${char.citizenid}</span></span></div>`; $('.characters-list').append(html); $('#char-' + i).data({"citizenid": char.citizenid, 'cData': char, 'cid': char.cid}); } else { html = `<div class="character" id="char-${i}" data-cid=""><span id="slot-name">Expediente Vacío<span id="cid"></span></span></div>`; $('.characters-list').append(html); }
    }
}
...
function setupCharInfo(cData, isInitial = false) {
    ...
    switch (cData.type) {
        ...
        case 'create':
            ...
            break;
        case 'info':
            if (char && char.charinfo) { newContentHtml = `<p id="char-info-titel">${char.charinfo.firstname}'s Dossier</p><div class="character-info-valid"><div class="character-info-box"><span id="info-label">Nombre:</span><span class="char-info-js">${char.charinfo.firstname} ${char.charinfo.lastname}</span></div><div class="character-info-box"><span id="info-label">Nacimiento:</span><span class="char-info-js">${char.charinfo.birthdate}</span></div><div class="character-info-box"><span id="info-label">Origen:</span><span class="char-info-js">${char.charinfo.nationality}</span></div><div class="character-info-box"><span id="info-label">Oficio:</span><span class="char-info-js">${char.job.label}</span></div><div class="character-info-box"><span id="info-label">Efectivo:</span><span class="char-info-js">$ ${char.money.cash}</span></div></div>`; } else { newContentHtml = `<p id="char-info-titel">Anotaciones</p><span id="no-char">Selecciona un expediente de la lista.</span>`; }
            break;
        default: // 'empty'
    ...
    if (cData.type === 'create' || cData.type === 'delete') {
        ...
        if (isInitial) { infoPage.html(newContentHtml); } else { infoPage.fadeOut(200, function() { $(this).html(newContentHtml).fadeIn(200); }); }
    }
}
...
function updateInvestigationPanels(raw) {
    ...
    if (!data || !selectedChar) { console.warn("[NUI] updateInvestigationPanels: No data or character selected."); return; }
    ...
    if (Array.isArray(data.houses) && data.houses.length > 0) {
        data.houses.forEach(house => { const houseName = house.houseid || 'Propiedad'; houseList.append(`<li>${houseName.charAt(0).toUpperCase() + houseName.slice(1)}</li>`); }); } else { houseList.html('<li>Ninguna conocida</li>'); }

    // Caballos
    const horseList = $('#info-horse-list').empty();
    if (Array.isArray(data.horses) && data.horses.length > 0) { data.horses.forEach(horse => { const horseName = horse.name || 'Caballo sin nombre'; const horseXP = horse.horsexp || 0; horseList.append(`<li>${horseName} <span class="horse-xp">(XP: ${horseXP})</span></li>`); }); } else { horseList.html('<li>Sin corceles notables.</li>'); }

    // Compañeros
    const companionList = $('#info-companion-list').empty();
    if (Array.isArray(data.companions) && data.companions.length > 0) { data.companions.forEach(companion => { const companionName = companion.companiondata?.name || 'Compañero sin nombre'; companionList.append(`<li>${companionName}</li>`); }); } else { companionList.html('<li>Ninguno conocido</li>'); }
    ...
    if (Array.isArray(data.outfits) && data.outfits.length > 0) { data.outfits.forEach(o => { outfitList.append(`<li class="outfit-item" data-outfit-name="${o.name}">${o.name}</li>`); }); } else { outfitList.html('<li>Sin atuendos guardados.</li>'); }

    // Skins
    const skinList = $('#skin-list').empty();
    if (Array.isArray(data.skins) && data.skins.length > 0) { data.skins.forEach((skin, index) => { skinList.append(`<li>Apariencia Guardada #${skin.id || (index + 1)}</li>`); }); } else { skinList.html('<li>Sin apariencias guardadas.</li>'); }
}

function clearInvestigationPanels() { $('.investigation-panel').hide(); $('.marker-menu').hide(); $('.page-marker').removeClass('active'); $('.notification-dot').hide(); }

function closeMulticharacterUI() {
    stopMusic();
    $('.container').fadeOut(250, function() { clearInvestigationPanels(); $('.sketchbook-wrapper').removeClass('is-active is-open'); $('.agent-toolkit').hide(); });
}
...
$(document).ready(function() {
    window.addEventListener('message', function(event) {
        ...
        switch (data.action) {
            case "ui":
                NChar = data.nChar;
                if (data.toggle) { $('.container').fadeIn(250); startUI(data.panelConfig, data.musicConfigData); } else { closeMulticharacterUI(); }
                break;
    ...
    $(document).on('input', '#volume-slider', function() {  let vol = $(this).val(); window.musicPlayer.audioPlayer.volume = vol; });
    
    // 3. LISTENERS DE LA INTERFAZ DE INVESTIGACIÓN
    $(document).on('click', '#main-marker', function(e) {
        e.stopPropagation();
        const menu = $('.marker-menu');
        if (menu.is(':visible')) { menu.slideUp(200); $('.investigation-panel').fadeOut(200); $('.marker-menu .page-marker').removeClass('active'); } else { menu.slideDown(200); }
    });
    ...
    $(document).on('click', '.marker-menu .page-marker', function(e) {
        ...
        if (panelClass === 'telegram-panel' && selectedChar) {
            ...
            if (recipient) { $.post('https://rsg-multicharacter/markTelegramsRead', JSON.stringify({ recipient: recipient })); }
        }
    });
    ...
    $(document).on('click', '.character', function(e) {
        ...
        if (cid) {  $.post('https://rsg-multicharacter/getInvestigationData', JSON.stringify({ citizenid: cid }), function(data) { updateInvestigationPanels(data); }); $("#play").text("Jugar"); $("#delete").show(); } else { $("#play").text("Crear"); $("#delete").hide(); }
        $("#play").show();
    });
    ...
    $(document).on('click', '#play', function(e) { 
        ...
        if (selectedChar.data('cid')) { $.post('https://rsg-multicharacter/selectCharacter', JSON.stringify({ cData: $(selectedChar).data('cData') })); } else { setupCharInfo({ type: 'create' }); } 
    });
    $(document).on('click', '#delete', function(e) {
        e.preventDefault(); 
        if (selectedChar && selectedChar.data('cid')) {  setupCharInfo({ type: 'delete', data: selectedChar.data('cData') }); } 
    });
    $(document).on('click', '#create', function (e) {
        e.preventDefault();
        $.post('https://rsg-multicharacter/selectCharacter', JSON.stringify({ cData: { cid: selectedChar.attr('id').replace('char-', '') } }));
    });
